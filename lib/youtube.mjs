// The latest videos of the ministry's two YouTube channels.
...
export const channelPages = new Map([
...
export async function requestFeedWithRetry(channelId, attempts = 3) {
...
// The cached list when it is fresh, a new one otherwise, and the old one again
// — marked stale — when YouTube does not answer. Resolves null for a channel
// that is not one of the ministry's.
export async function latestVideos(channelId) {
  if (!channelPages.has(channelId)) return null;
  const cached = feedCache.get(channelId);
  if (cached && Date.now() - cached.createdAt < cacheLifetime) return cached.payload;
  try {
    const videos = await requestFeedWithRetry(channelId);
