...
import { normalizePost, sortPosts } from '../src/posts.js';
...
const POST_COLUMNS = 'id,title,body,church_id,highlighted,highlight_until,published_at,author_id,post_images(url,caption,sort_order)';
...
export function createPublicData({
  ...
  return {
    ...
    posts({ fresh = false } = {}) {
      if (!configured) return Promise.resolve({ posts: [], updatedAt: stamp(), source: 'sem-base' });
      if (fresh) forget('posts');
      return remember('posts', async () => {
        const [rows, authors, reactions, comments, directory] = await Promise.all([
        ...
        const posts = sortPosts((rows || []).map((row) => normalizePost(row, { authors: byId, reactions, comments, churches: directory.churches })));
        return { posts, updatedAt: stamp(), source: 'supabase' };
      });
    },
