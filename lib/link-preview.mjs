...
import { churchTitle, findChurch, SEAT_LABELS, serviceLabel } from '../src/directory.js';
import { escapeHtml } from '../src/html.js';
import { videosIn } from '../src/posts.js';
import { formatDuration } from '../src/prayers.js';
...
const SECTIONS = [
  ...
  [/^\/igrejas\/?$/, { title: 'Igrejas ISTN-SJ pelo mundo', description: 'Encontre a sua igreja local: igrejas, casas de oração e igrejas online, com os dias de culto e os endereços.', preload: PROPHET.churches }],
  [/^\/igrejas\//, { title: 'Igrejas ISTN-SJ pelo mundo', description: 'Encontre a sua igreja local: igrejas, casas de oração e igrejas online, com os dias de culto e os endereços.' }],
  ...
  [/^\/pessoas(\/|$)/, { title: 'Quem serve e quem escreve na ISTN-SJ', description: 'A página de quem publica na ISTN-SJ: a função na igreja e os anúncios que deixou.' }],
  [/^\/oracoes(\/|$)/, { title: 'Orações do Profeta Elias, por tema', description: 'Orações gravadas para o que as pessoas estão a viver: doença, libertação, finanças. Encontre pelo tema, ouça, e envie a quem precisa.' }]
];
...
export function churchPreview(church) {
  ...
  return {
    ...
    title: `${churchTitle(church)}${church.seat ? ` · ${SEAT_LABELS[church.seat]}` : ''}`,
    description: clip([
...
// A prayer shared as a link: whoever receives it sees what it is before
// opening, which matters when what arrives is a prayer for someone who is ill.
export function prayerPreview(prayer) {
  return {
    title: `${prayer.title} · Oração do Profeta Elias`,
    description: clip([prayer.theme?.name, prayer.description, formatDuration(prayer.duration)].filter(Boolean).join(' — ')) || DEFAULT.description,
    image: DEFAULT.image
  };
}

// The preview for `pathname`. `load` gives the directory, the posts and the
// prayers; it is only called for the addresses that need them.
export async function previewFor(pathname, load = {}) {
  ...
  const prayer = pathname.match(/^\/oracoes\/([\w-]+)\/?$/);
  if (prayer && load.prayers) {
    const found = (await load.prayers()).prayers.find((item) => item.id === prayer[1]);
    if (found) return prayerPreview(found);
  }
